...
from core.srt_splitter import split_segments_by_limit
...
async def generate_srt(audio_path: Path, context: Optional[str] = None, 
    ...
    Args:
        ...
        context: Contexte optionnel pour la transcription
        start_time: Début en secondes (None = début du fichier)
        end_time: Fin en secondes (None = fin du fichier)
    ...
    if max_words is not None or max_chars is not None:
        ...
        print(f"✅ {len(aligned)} segments après division")
